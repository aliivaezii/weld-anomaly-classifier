...
------
  ...
  Phase 2 (steps 7–11):  Tabular baseline -> WeldFusionNet training
  Phase 3 (steps 12–15): Calibration -> evaluation -> inference -> post-hoc calibration
...
-----
  ...
  python -m pipeline.run_all --steps 1 2 3 4 5 6 7 11 12 13 14 15

  # Phase 2/3 only (after Phase 1 is done)
  python -m pipeline.run_all --steps 7 11 12 13 14 15

  # Just train + evaluate
  python -m pipeline.run_all --steps 11 13

  # Just post-hoc calibration on existing predictions
  python -m pipeline.run_all --steps 15
...
from pipeline import step15_postprocess
...
STEPS = {
    ...
    15: ("Step 15: Post-hoc calibration",         step15_postprocess),
}
...
def main():
    ...
    for step_num in args.steps:
        if step_num not in STEPS:
            print(f"\n  WARNING: Step {step_num} not registered (skipping)")
            continue
