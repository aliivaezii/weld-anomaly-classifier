...
def train_one_epoch(model, loader, criterion, optimizer, scheduler, device, grad_clip):
    ...
    for batch_idx, batch in enumerate(loader, 1):
        batch_t0 = time.time()
        sensor = batch["sensor"].to(device) if model.use_sensor else None
        ...
        if batch_idx == 1 or batch_idx % 50 == 0 or batch_idx == total_batches:
            ...
            print(f"\r    [Train] {batch_idx:>4}/{total_batches}  "
                  f"loss={avg_loss:.4f}  "
...
def validate(model, loader, criterion, device):
    ...
    for batch_idx, batch in enumerate(loader, 1):
        ...
        if batch_idx == 1 or batch_idx % 20 == 0 or batch_idx == total_batches:
            ...
            eta = (elapsed / batch_idx) * (total_batches - batch_idx)
            print(f"\r    [Val]   {batch_idx:>4}/{total_batches}  "
...
def run(config_path="config.yaml", use_video=False, use_sensor=True):
    ...
    # ── MPS / GPU optimizations ──
    if device.type == "mps":
        # Enable float32 matmul precision for M-series (uses AMX/Neural Engine)
        if hasattr(torch, "set_float32_matmul_precision"):
            torch.set_float32_matmul_precision("high")
            print("  Float32 matmul precision: high")

    # torch.compile — DISABLED on MPS due to stride assertion errors
    # in convolution_backward (inductor backend bug on Apple Silicon).
    # Safe on CUDA; skip entirely on MPS/CPU.
    compiled = False
    if hasattr(torch, "compile") and device.type == "cuda":
        ...
        print(f"  torch.compile: skipped (device={device.type},"
              f" MPS has inductor stride bugs)")
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        print(f"    Epoch: {epoch_elapsed/60:.1f}min  "
              f"Total: {total_elapsed/60:.1f}min  "
              f"ETA: ~{eta_total/60:.0f}min ({epochs_left} epochs left)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-sensor", action="store_true",
                        help="Exclude sensor (CSV) branch — audio+video only")
    args = parser.parse_args()

    logging.basicConfig(level=logging.WARNING, format="%(levelname)s: %(message)s")
    run(args.config, use_video=args.video, use_sensor=not args.no_sensor)
