...
class TinyCNNVideoEncoder(nn.Module):
    ...
    def forward(self, x):
        """x: (B, T, 3, H, W)"""
        B, T = x.shape[:2]
        flat = x.contiguous().reshape(B * T, *x.shape[2:])          # (B*T, 3, H, W)
        feats = self.frame_cnn(flat).squeeze(-1).squeeze(-1)  # (B*T, 64)
        feats = feats.contiguous().reshape(B, T, -1)                 # (B, T, 64)
...
class MobileNetVideoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        feats = feats.contiguous().reshape(B, T, -1)         # (B, T, 576)
...
class WeldFusionNet(nn.Module):
    ...
    use_sensor : bool        – include sensor (CSV) branch
    ...
    At minimum ONE of (use_sensor, use_video) must be True in addition
    to the always-present audio branch.
    """
    ...
    def __init__(
        ...
        if use_sensor:
            self.sensor_encoder = Conv1dEncoder(in_channels=26, embed_dim=sensor_dim)
        else:
            self.sensor_encoder = None

        self.audio_encoder = Conv1dEncoder(in_channels=18, embed_dim=audio_dim)
        ...
        # ── Compute fusion input dim dynamically ──
        total_dim = audio_dim                            # audio is always present
        if use_sensor:
            total_dim += sensor_dim
        if use_video:
            total_dim += video_dim
    ...
    def forward(self, sensor, audio, video=None):
        ...
        Parameters
        ----------
        sensor : (B, 26, 25) or None — ignored when use_sensor=False
        audio  : (B, 18, 25)
        ...
        """
        # ── Encode each active modality ──
        embeddings = []

        if self.use_sensor and self.sensor_encoder is not None:
            s = self.sensor_encoder(sensor)          # (B, 64)
            embeddings.append(("sensor", s))

        a = self.audio_encoder(audio)                # (B, 64)
        embeddings.append(("audio", a))

        if self.use_video and self.video_encoder is not None and video is not None:
            v = self.video_encoder(video)            # (B, 128)
            embeddings.append(("video", v))

        # ── Fuse ──
        if self.fusion_type == "transformer":
            projected = []
            for name, emb in embeddings:
                if name == "sensor" and self.sensor_proj is not None:
                    projected.append(self.sensor_proj(emb))
                elif name == "audio":
                    projected.append(self.audio_proj(emb))
                elif name == "video" and self.video_proj is not None:
                    projected.append(self.video_proj(emb))
            fused = self.fusion(*projected)
        else:
            raw = [emb for _, emb in embeddings]
...
def build_model(cfg, use_sensor=True, use_video=False):
    ...
    print(f"  Model: WeldFusionNet (fusion={fusion_type}, modalities={'+'.join(modalities)})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-sensor", action="store_true",
                        help="Exclude sensor (CSV) branch")
    ...
    model = build_model(cfg, use_sensor=use_sensor, use_video=args.video)
    print(f"\n  Architecture:\n{model}\n")

    # Dummy forward pass
    B = 4
    sensor = torch.randn(B, 26, 25) if use_sensor else None
